...
class Table extends React.Component {
  ...
  renderToggle(node) {
    ...
    if (hasVisibleChildren) {
      return <ToggleAnchor collapsed={node.collapsed()} node={node} hiddenDeep/>;
    }
  ...
  renderTbody(nodes) {
    ...
    const items = nodes.map(node =>
      <tr // eslint-disable-line
        ...
        ref={domNode => { node.itree.ref = domNode; }} // eslint-disable-line
      >
        {columns.map((item, index) => {
          const key = `${item.field || index}-${node.id}`;
          if (index) {
            const content = isFunction(item.template) ? item.template(node) : node[item.field];
            return <td key={key}>{content}</td>;
          }
          const layer = node.getTextualHierarchy().length - 1;
          return (
            <td key={key}>
              {layer ? <span className={`space-holder space-${layer}x`}></span> : null}
    ...
    while (++index < length) {
      ...
      if (depth > 0 && predicate(value.children)) {
        if (depth > 1) {
          ...
          this.flattenDeep(value.children, depth - 1, predicate, isStrict, result);
        } else {
