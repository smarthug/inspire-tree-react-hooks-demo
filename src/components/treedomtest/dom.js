 ...
 */
export default class InspireTreeDOM extends React.Component {
  ...
  attach(target) {
    ...
    if (this.config.deferredRendering || this._tree.config.deferredLoading) {
      ...
      this._tree.config.pagination.limit = limit > 0
        ? limit
  ...
  keyboardListener(event) {
    ...
    if ([keys.DOWN_ARROW, keys.ENTER, keys.LEFT_ARROW, keys.RIGHT_ARROW, keys.UP_ARROW]
   ...
   */
  renderNodes(nodes) {
    ReactDOM.render(<Tree dom={this} nodes={nodes || this._tree.nodes()} />, this.$target);
  }
  ...
  scrollListener(event) {
    if (!this.rendering && !this.loading) {
      ...
      _.each(links, (link) => {
        ...
        const overlap = !(rect.right < r.left || rect.left > r.right
